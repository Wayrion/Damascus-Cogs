...
class Cocktail(commands.Cog):
    ...
    async def name(self, ctx, *, query):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"http://www.thecocktaildb.com/api/json/v1/1/search.php?s={query}"
            ) as resp:
    ...
    async def firstletter(self, ctx, *, query):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"http://www.thecocktaildb.com/api/json/v1/1/search.php?f={query}"
            ) as resp:

                response = await resp.json()
                data = response.get("drinks")
                embed = discord.Embed(
                    title=f"List of all cocktails starting with {query}",
    ...
    async def ingredientname(self, ctx, *, name):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://www.thecocktaildb.com/api/json/v1/1/search.php?i={name}"
            ) as resp:
    ...
    async def cocktailid(self, ctx, id):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"http://www.thecocktaildb.com/api/json/v1/1/lookup.php?i={id}"
            ) as resp:
    ...
    async def ingredientid(self, ctx, name):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://www.thecocktaildb.com/api/json/v1/1/lookup.php?iid={name}"
            ) as resp:
    ...
    async def alcohol(self, ctx, *, query):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"http://www.thecocktaildb.com/api/json/v1/1/filter.php?a={query}"
                ) as resp:
                    ...
                    if data != None:
                        for i in data:
                            ...
                            Cocktails.append(a)
                            Pages.append(
                                multipledrinksformatter(i, list(data).index(i))
                            )
            ...
            await ctx.send(
                "Invalid parameters. Please use `!cocktail list alcoholic` to see all alcoholic filters"
            )
    ...
    async def category(self, ctx, *, query):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://www.thecocktaildb.com/api/json/v1/1/filter.php?c={query}"
                ) as resp:

                    response = await resp.json()
                    data = response.get("drinks")
                    embed = discord.Embed(
                        title=f"List of all {query} cocktails",
                        color=discord.Color.random(),
                    )
                    Pages = []
            ...
            await ctx.send(
                "Invalid parameters. Please use `!cocktail list categories` to see all alcoholic filters"
            )
    ...
    async def glass(self, ctx, *, query):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://www.thecocktaildb.com/api/json/v1/1/filter.php?g={query}"
                ) as resp:

                    response = await resp.json()
                    data = response.get("drinks")
                    embed = discord.Embed(
                        title=f"List of all cocktails that use {query}",
                        color=discord.Color.random(),
                    )
                    Pages = []
            ...
            await ctx.send(
                "Invalid parameters. Please use `!cocktail list glasses` to see all alcoholic filters"
            )
    ...
    async def incocktail(self, ctx, query):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"https://www.thecocktaildb.com/api/json/v1/1/search.php?i={query}"
            ) as resp:
