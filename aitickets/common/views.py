...
log = logging.getLogger("red.wayrion.supportview")
    ...
    async def closeticket(self, interaction: Interaction, button: Button):
        ...
        if requires_reason:
            ...
            try:
                ...
                log.warning(
                    f"Failed to send ticket modal for panel {panel_name}", exc_info=e
                )
    ...
    async def create_ticket(self, interaction: Interaction):
        ...
        if required_roles := panel.get("required_roles", []):
            if not any(r.id in required_roles for r in user.roles):
                roles = [
                    guild.get_role(i).mention
                    for i in required_roles
                    if guild.get_role(i)
                ]
                em = discord.Embed(
                    description=_(
                        "You must have one of the following roles to open this ticket: "
                    )
                    + humanize_list(roles),
                    color=discord.Color.red(),
                )
        ...
        async def delete_delay():
            desc = _("Your ticket has been created! {}").format(
                channel_or_thread.mention
            )
