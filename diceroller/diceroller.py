...
class DiceRoller(commands.Cog):
    ...
    async def fudge(self, ctx, argument, *, reason: str = None):
        ...
        try:
            ...
            if number <= 50000:
                ...
                await ctx.send(
                    f"{ctx.message.author.mention} rolled {reason}, with the argument `{argument}` and got a total of `{total}`"
                )
                if len(results) > 50:
                    await menu(
                        ctx=ctx, pages=self.chunks(results), controls=DEFAULT_CONTROLS
                    )
                else:
            ...
            await ctx.send(
                f"Invalid input. Please follow the format: `{list(await self.bot.get_prefix(ctx.message))[0]}fudge <number>df<modifier> [reason]`"
            )
    ...
    async def roll(self, ctx, argument, *, reason: str = None):
        ...
        try:
            ...
            await ctx.send(
                f"Invalid input. Please follow the format: `{list(await self.bot.get_prefix(ctx.message))[0]}roll <number>df<modifier>` [reason]"
            )
