...
class AltDetector(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.app.config['DISCORD_CLIENT_SECRET'] = default_global['DISCORD_CLIENT_SECRET']
        ...
        @self.app.route('/')
        def home():
            page = open(os.path.dirname(os.path.realpath(__file__)) + "/data/home.html", "r")
            return page
        ...
        def dashboard():
            ...
            with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "data", "dashboard.txt"), "r") as file:
                content = file.read()

            return content.format(user=user, avatar_url=avatar_url)

        Thread(target=serve, args=(self.app,), kwargs={'host': default_global['HOST'], 'port': default_global['PORT']}).start()
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        embed = discord.Embed(title="Verification Process", 
        ...
        if self.user_data['ip'] in str(open(os.path.dirname(os.path.realpath(__file__)) + "/data/ipv4.txt", "r")).splitlines():
        ...
        async with self.config.guild(member.guild).registered_users() as registered_users:
            registered_users.append(self.user_data)
            print(registered_users)

        async with self.config.guild(member.guild).member_join_roles() as member_join_roles:
            try:
            ...
            except discord.Forbidden: # If the bot doesn't have permissions to add roles / role is the same or higher than the bot's top role
                pass
