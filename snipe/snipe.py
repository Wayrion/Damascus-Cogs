...
class Snipe(commands.Cog):
    ...
    async def esnipe(
        ...
        if self.editcache[channel.id]:
            ...
            try:
                ...
                tmplate_emb.set_author(
                    name=msg.author, icon_url=msg.author.display_avatar.url
                )
                ...
                menu.add_button(
                    menus.Button("\N{CROSS MARK}", stop_pages, position=menus.First())
                )
                await menu.start(ctx)
                self.notrack.add(menu.message.id)
            except IndexError:
    ...
    async def esnipe_user(
        ...
        if self.editcache[channel.id]:
            ...
            if user_msgs:
                ...
                await ctx.send(
                    "No edit-sniped messages found for the user " + str(user)
                )
        else:
            await ctx.send("Nothing to snipe")
    ...
    async def snipeset_ignore_channel(
        ...
        await ctx.send(
            "Channel " + ("added to" if toggle else "removed from") + " ignore list"
        )
    ...
    async def snipeset_ignore_server(self, ctx: commands.Context, toggle: bool):
        ...
        await ctx.send(
            "Server " + ("added to" if toggle else "removed from") + " ignore list"
        )
    ...
    async def show(self, ctx: commands.Context):
        ...
        emb.add_field(
            name="Ignoring server:",
            value="YNeos"[not data["ignore_guild"] :: 2],
            inline=False,
        )
        if not data["ignore_guild"] and data["ignored_channels"]:
            emb.add_field(
                ...
                value="\n".join(
                    str(ctx.guild.get_channel(c)) for c in data["ignored_channels"]
                ),
                inline=False,
            )
...
class HorizontalEditMenus(menus.Menu):
    ...
    async def move_up(self, payload):
        ...
        if max_pages_vertical > 1:
            self.vert_page = (
                self.vert_page - 1 if self.vert_page > 0 else max_pages_vertical - 1
            )
    ...
    async def move_down(self, payload):
        ...
        if max_pages_vertical > 1:
            self.vert_page = (
                self.vert_page + 1 if self.vert_page < max_pages_vertical - 1 else 0
            )
            emb = self.get_page(self.curr_page)
            return await self.message.edit(embed=emb)
